<template>
        ...
        <TransitionGroup name="list" v-if="listOfPage.length">
            <BListGroupItem v-for="item of listOfPage" :key="item.id">
                <div>
                    ...
                    <button @click="removeGood({ id: item.id, type: item.type })">下架</button>
    ...
    <BModal v-model="modalActived" title="產品資訊" @hide="hideModal">
        <template #default>
            <div>
                <p>類型：<select v-model.number="modifiedData.type_id">
                        <template v-for="item of myStoreStore.good.types">
                            <option :value="item.id" :disabled="currentData.type_id === item.id">{{ item.zh }}
                            </option>
                        </template>
                    </select>
                </p>
                <BFormGroup label="名稱" label-for="name" :state="modifiedDataStatus.name.valid"
                    :invalid-feedback="modifiedDataStatus.name.message">
                    <BFormInput id="name" v-model.trim="modifiedData.name" :placeholder="currentData.name"
                        @input="modifiedDataStatus.name.inputed = true" />
                </BFormGroup>
                <BFormGroup label="價格" label-for="price" :state="modifiedDataStatus.price.valid"
                    :invalid-feedback="modifiedDataStatus.price.message">
                    <BFormInput id="price" v-model.number="modifiedData.price" :placeholder="currentData.price"
                        @input="modifiedDataStatus.price.inputed = true" />
                </BFormGroup>
                <BFormGroup label="數量" label-for="total" :state="modifiedDataStatus.total.valid"
                    :invalid-feedback="modifiedDataStatus.total.message">
                    <BFormInput id="total" v-model.number="modifiedData.total" :placeholder="currentData.total"
        ...
        <template #footer>
            <BButton type="submit" variant="primary" :disabled="submitDisabled" @click="modifyGood">修改</BButton>
...
const fillCount = computed(() => {
    ...
    console.log('listOfPage.value.length', listOfPage.value.length)
    console.log(listOfPage.value.length % props.num)
...
const currentData = computed(() => {
    ...
    if (!id) {
        ...
        let { type_id, name, price, total } = myStoreStore.good.list.find(item => item.id === id)
...
async function removeGood({ id, type }) {
    if (!confirm('確定要刪除？')) { return }
    const payload = { id, type_id: type.id }
    const { valid } = await validate.removeMyGood(payload)
    let msg = null
    if (!valid) {
        msg = '發生未知錯誤'
    } else {
        msg = await myStoreStore.requestRemove(payload)
    }
    if (msg) {
        statusStore.reload('發生未知錯誤')
    } else {
        alert('商品下架成功')
    }
}

async function modifyGood() {
    const payload = await _validatemodifiedDataAndGetPayload()
    let msg = null
    if (!payload) {
        msg = '發生未知錯誤'
    } else {
        msg = await myStoreStore.requestModify(payload)
    }
    if (msg) {
...
async function _validatemodifiedDataAndGetPayload() {
    ...
    const validRes = await validate.modifyMyGood({ ...payload, _old: currentData.value })
