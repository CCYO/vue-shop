...
function _genSchema({ host, name, json }) {
  return {
    ..._init(json),
    $id: `${host}/${name}.json`
  };
...
function _addErrorMessage(json) {
  ...
  for (let key in res) {
    ...
    if (key === "minLength" || key === "maxLength") {
      if (![res.minLength, res.maxLength].some((item) => typeof item !== "number")) {
        MSG.minLength = MSG.maxLength = `必須介於${res.minLength}-${res.maxLength}個字符`;
      ...
      if (![res.minimum, res.maximum].some((item) => typeof item !== "number")) {
...
exports.default = _genSchema;
//# sourceMappingURL=_genSchema.js.map
